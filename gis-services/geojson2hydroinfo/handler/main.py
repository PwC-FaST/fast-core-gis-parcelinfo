...
def handler(context, event):
    ...
    try:
        ...
        collection = FunctionConfig.hydro_collection
        client = FunctionState.mongodb_client

        # MongoDB SOC collection
        hydro = client[db][collection]
        ...
        # Get hydro features intersecting the target feature
        hydro_features = list(hydro.find({
            "geometry": { 
                "$geoIntersects": { 
        ...
        # Reprojection of hydro geometries in legal CRS
        hydro_geoms = [
            transform(reproject,shape(h['geometry']))
            for h in hydro_features
        ]

        # Reprojection of target geometry in legal CRS
        parcel = transform(reproject,shape(feature['geometry']))

        for i, f in enumerate(hydro_features):
            hydro_geoms[i].value = f

        result = []
        for h in hydro_geoms:
            d = parcel.distance(h)
            if d <= search_distance:
                r = {
                    ...
                    "intersects": False,
                    "minDistance": d,
                    "properties": h.value['properties']['hydro'],
                    "wktType": h.type.upper()
                }
                r.update(h.value['properties'])
                r.pop('crs')
                r.pop('hydro')
                r.pop('version')
                if parcel.intersects(h):
                    i = parcel.intersection(h)
                    r['intersects'] = True
                    r['intersection'] = i.area if i.area else i.length
                result.append(r)

        context.logger.info("'{0}' hydro features processed".format(len(hydro_features)))
    ...
    return context.Response(body=json.dumps({ 'hydro': result }),
                            headers={},
...
class FunctionConfig(object):
    ...
    hydro_collection = None

    default_search_distance = None # defaul distance to search hydro features within (in meters)
...
class Helpers(object):
    ...
    def load_configs():
        ...
        FunctionConfig.hydro_collection = os.getenv('HYDRO_MONGODB_COLLECTION')
        FunctionConfig.default_search_distance = int(os.getenv('HYDRO_DEFAULT_SEARCH_DISTANCE',100))
